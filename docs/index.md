# TaskFlow Help Center

Find setup guides, step-by-step instructions, admin settings, and troubleshooting articles.

## Recommended reading order (new users)

1. [Create an account](getting-started/create-account.md)
2. [Create your first project](getting-started/first-project.md)
3. [Invite teammates](getting-started/invite-team.md)
4. [Create tasks](guides/create-tasks.md)

---

## Getting started

- [Create an account](getting-started/create-account.md)
- [Create your first project](getting-started/first-project.md)
- [Invite teammates](getting-started/invite-team.md)

## Guides (how to use TaskFlow)

- [Create tasks](guides/create-tasks.md)
- [Assign tasks and use @mentions](guides/assign-and-mentions.md)
- [Manage notifications](guides/notifications.md)

## Admin and security

- [Roles and permissions](admin/roles-and-permissions.md)
- [SSO and security settings](admin/sso-and-security.md)

## Troubleshooting

- [Troubleshoot login issues](troubleshooting/login-issues.md)
- [Fix “email invite not received”](troubleshooting/email-invites-not-received.md)
- [Fix “search not returning results”](troubleshooting/search-not-working.md)
- [Fix “not receiving notifications”](troubleshooting/not-receiving-notifications.md)
- [Fix “tasks not visible to teammates” (permissions)](troubleshooting/tasks-not-visible-to-teammates.md)

## FAQs

- [Billing FAQ](faqs/billing-faq.md)

## Release notes

- [Release notes: 2026-01](release-notes/2026-01.md)
- [Release notes template](release-notes/template.md)